...
class CompanyTest < ActiveSupport::TestCase
  test 'cannot be empty' do
    assert Company.new.invalid?, 'company is valid without name, city and address'
  end
  ...
  test 'has name address domain and office' do
    company = Company.new(name: 'Rinity', address: 'Over the rainbow', domain:'_rinity.com', city: 'Debrecen', offices_attributes:[{ name: 'HQ', address: 'Archam', city: 'Gotham City' }])
    assert company.valid?, company.errors.full_messages
  end
  test 'company has many employees' do
    company = Company.new(name: 'Rinity', address: 'Over the rainbow', domain: '_rinity.com', city: 'Debrecen')
    assert company.valid?, company.errors.full_messages
